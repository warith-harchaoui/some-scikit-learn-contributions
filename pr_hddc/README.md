...
> **Reviewer companion:** [`docs/HDDC.md`](../docs/HDDC.md) is the deep-dive reference for this PR — naming-scheme tradeoffs (paper bracket vs geometric code vs per-axis kwargs), the per-row parameter-count audit against Bouveyron 2007 Table 1, and the Cattell scree rule for `d_k` (algorithm derived from the equations in Bouveyron et al. and the published behaviour of `HDclassif`; default threshold differs).
>
> **License note.** This implementation is original Python written against the published equations in Bouveyron, Girard & Schmid (2007) and the [`HDclassif`](https://CRAN.R-project.org/package=HDclassif) help-page algorithm description. **No source code was copied from `HDclassif`.** `HDclassif` (GPL-2) is used only as an external numerical reference in the optional [`hdclassif_parity/`](../hdclassif_parity/) check (BSD-3 ↔ GPL-2 isolated by process boundary: R is invoked as a subprocess, CSVs are exchanged on disk, no linking).
>
